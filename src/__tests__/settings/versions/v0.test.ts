...
describe('triggerSchema', () => {
    ...
    test('throw no error if regex does not end with $', () => {
        // This was a bug in v0 but needs to be supported for backwards compatibility.
        const data = {type: 'regex', value: '\\d+'};
        expect(triggerSchema.parse(data)).toEqual(data);
    });
});
